...
// Initial Default Topic Ratios (75% Tech & AI, 25% Edukasi & Alam)
...
/**
 * Step 2 Workflow: Generate 4 Content Topic Options based on Topic Ratios
 */
...
/**
 * Step 3 Workflow (ACC Approval): Triggered ONLY when user clicks "ACC / Disetujui"
 * Generates Full Script, Captions, Veo 3.1 Prompts + Pricing, and saves directly to Supabase Calendar!
 */
export async function approveAndGenerateContent(selectedOption: ContentRecommendationOption): Promise<ApprovedContentResult> {
  // Call Gemini for full content generation
  const plan: GeneratedContentPlan = await generateContentPlanWithAI(selectedOption.title);

  // Veo 3.1 Video pricing calculator (8 seconds as standard)
  const durationSec = 8;
  const veoCost = calculateVeoVideoCost(durationSec);

  // Save approved post directly into Supabase content_posts table
  const newPostData: Omit<ContentPostItem, "id"> = {
    ...
    scheduled_date: getTodayLocalDateString(),
